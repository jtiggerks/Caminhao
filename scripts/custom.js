...
$.ajax({
		url: 'http://www.radiometropole.com.br/app/ping.php?nocache=' + (new Date()).getTime(),
		cache:false,
		dataType: 'jsonp',
		jsonp: 'callback',
		timeout: 5000,
...
$.ajax({
    url: 'http://www.radiometropole.com.br/app/func-mensagem.php?nocache=' + (new Date()).getTime(),
    type: 'POST',
	cache:false,
...
$.ajax({
	...
	success: function(results){
	$.ajax({
		url : "http://www.radiometropole.com.br/app/func-vcreporter.php",
		type : "post",
		crossDomain: true,
		timeout: 30000,
		processData: true,
...
$.ajax({
	url: 'http://www.radiometropole.com.br/app/func-infotrack.php?track='+track+'&nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
...
$('#pagina-atual').val('radio');
$('.header-controls').hide();
$("#voltar-header").hide();
	e.preventDefault();
	var hideDeffered = $('#notificacoes, #podcast, #home, #podcast-player, #noticias, #abre-noticia, #mensagem, #vc-reporter').hide("fade", { direction: "up", easing: 'easeInOutBack' }, 50);
	hideDeffered.promise().done(function() {
		
		$('#titulo-header').text("");
		$('#radio').show("fade", { direction: "right", easing: 'easeInOutBack' }, 1000, function(){
			...
			$.ajax({
				url: 'http://www.radiometropole.com.br/app/func-programacao.php?nocache=' + (new Date()).getTime(),
				cache:false,
				dataType: 'jsonp',
				jsonp: 'callback',
				timeout: 10000,
				success: function(results){
					...
					for (var i = 0; i < arrayLength; i++) {
						if(results.data[i].status != null){
							if(results.data[i].status == 1){ //Se retornou positivo
							$('.nome-programa').html(results.data[i].titulo);
							}
						}
					}	
				},
				error: function() {
					$('.nome-programa').html('Sem conexão');
				}
			});
...
function abre_Podcast(){
$('.header-controls').show();
$("#voltar-header").hide();
$('.ordena-podcast').html('');
var hideDeffered = $('#notificacoes, #home, #radio, #podcast-player, #noticias, #abre-noticia, #mensagem, #vc-reporter').hide("fade", { direction: "up", easing: 'easeInOutBack' }, 50);
	hideDeffered.promise().done(function() {
		$('#titulo-header').html("<div style='margin-left: -30px;height: 35px;'>Podcast</div>");
		$('#podcast').show("slide", { direction: "up", easing: 'easeInOutBack' }, 1000, function() {
					pega_Podcast();	
				});
		});
	}
...
$("#voltar-header").hide();
	...
	hideDeffered.promise().done(function() {
		if($('#programa option').size() == 1){ // Se só tiver 1 option no select, pegue o restante.
			$.ajax({
				url: 'http://www.radiometropole.com.br/app/func-programas.php?nocache=' + (new Date()).getTime(),
				cache:false,
				dataType: 'jsonp',
				jsonp: 'callback',
				timeout: 10000,
				success: function(results){
					...
					for (var i = 0; i < arrayLength; i++) {
						if(results.data[i].status != null){
							if(results.data[i].status == 1){ //Se retornou positivo
								$("#programa")[0].options.add( new Option(results.data[i].titulo,results.data[i].id));
							}
						}
...
hideDeffered.promise().done(function() {
	...
	$('#noticias').show("fade", { direction: "right", easing: 'easeInOutBack' }, 1000,function(){
		if($("#status-noticia").val() != 1){
			...
			$.ajax({
				url: 'http://www.radiometropole.com.br/app/func-destaque.php?nocache=' + (new Date()).getTime(),
				cache:false,
				dataType: 'jsonp',
				jsonp: 'callback',
				timeout: 10000,
...
$.ajax({
	url: 'http://www.radiometropole.com.br/app/func-noticia.php?t='+t+'&nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
...
$.ajax({
	url: 'http://www.radiometropole.com.br/app/func-detalhe-noticia.php?id='+id+'&nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
...
$('.img_like').on('click', function() {
		...
		if(tem > -1) {
			...
			likes.splice(i, 1);
			var urllike = 'http://www.radiometropole.com.br/app/func-like.php?id='+id+'&t=d&nocache=';
		}
		else{
			...
			var urllike = 'http://www.radiometropole.com.br/app/func-like.php?id='+id+'&t=s&nocache=';
		}
...
$.ajax({
	url: 'http://www.radiometropole.com.br/app/func-podcast.php?nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
...
function playRadio(){
	...
	var track = 'http://audio.radiometropole.net.br:8000/;stream.mp3';
    soundManager.onready(function() {
        audio = soundManager.createSound({
            id: 'playerRadio',
			debugMode: true,
...
$.ajax({
	url: 'http://www.radiometropole.com.br/app/func-buscanoticia.php?texto='+noticia+'&nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
...
$('#resultado').html('<div class="sidebar-divider side-resultado"><img src="imagens/loader-busca.gif" style="margin:0 auto;"/></div>');	
$.ajax({
	url: 'http://www.radiometropole.com.br/app/func-buscapodcast.php?termo='+termo+'&nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
